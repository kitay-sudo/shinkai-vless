...
| --- | --- | --- |
| [`install.sh`](install.sh) | Поднимает VLESS + Reality сервер. Клиент коннектится напрямую. | Обычный заграничный VPS. |
| [`install-relay.sh`](install-relay.sh) | Поднимает транзитный узел в РФ, который форвардит трафик на основной сервер. | Когда провайдер режет прямой коннект и нужен «белый» российский IP на входе. |

```text
install.sh:        клиент ───────────────────────────────→ сервер → интернет
install-relay.sh:  клиент → РФ-узел (белый IP) → сервер (Швеция) → интернет
```
...
- Цепочка через РФ — отдельный скрипт, а не флаг-комбайн: захотел — добавил узел, не захотел — пользуешься прямым подключением.
...
## Цепочка через РФ-сервер — `install-relay.sh`
...
На основном сервере (Швеция) **менять ничего не нужно** — РФ-узел подключается к нему как обычный клиент с уже существующими ключами.
...
| --- | --- | --- |
| `RELAY_PORT` | Порт входа `клиент → РФ` | `443` |
| `RELAY_SNI` | SNI-маска входа | `ozon.ru` |
| `SWEDEN_LINK` | `vless://` ссылка основного сервера | — |

Порт `443/tcp` выбран по умолчанию специально — он максимально похож на обычный HTTPS. Не забудь открыть его в фаерволе/security group провайдера.
...
РФ-узел умеет работать в двух режимах, и переключение не требует менять клиентскую ссылку — меняется только сторона сервера:

```bash
sudo vless-mode direct   # ходишь начисто, выход прямо с РФ-сервера (российский IP, быстро)
sudo vless-mode chain    # трафик идёт через основной сервер (Швеция)
sudo vless-mode status   # показать текущий режим
```

- `direct` — для сервисов, которые в РФ не заблокированы: меньше задержка, российский IP, но провайдер всё равно не видит твой трафик (клиент → РФ шифруется Reality).
- `chain` — когда нужен обход: выход через Швецию.

После установки активен режим `chain`. Команда сама перезапускает Xray.

**Добавить `vless-mode` на уже работающий релей** (без переустановки и смены ключей):

```bash
sudo curl -sSL https://raw.githubusercontent.com/kitay-sudo/shinkai-vless/main/vless-mode -o /usr/local/bin/vless-mode
sudo chmod +x /usr/local/bin/vless-mode
```

Конфиг релея уже содержит оба выхода, поэтому переустанавливать ничего не нужно — клиентская ссылка остаётся прежней.

### Если связь нестабильна

Обе ноды используют `xtls-rprx-vision`. Это влияет только на «работает / не работает», а не на безопасность. Если на конкретной версии Xray цепочка капризничает, убери `flow` у входного клиента (в `clients[0]` конфига РФ и в клиентской ссылке) — Reality продолжит работать без Vision.
